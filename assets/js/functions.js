...
let stage = {
    ...
    doAttack(attacking, attacked){
        if (attacking.life <= 0 || attacked.life <= 0) {
            log.addMessage('Alguem esta morto...');
            return;
        }
        ...
        }else if(actualAttack > actualDefense && actualDefense < 0){
            attacked.life -= (actualAttack - actualDefense);
            log.addMessage(`${attacked.name} conseguiu defender mais teve um pequeno dano de ${actualAttack}`);
        }else{
